...
#pragma exclude_renderers d3d11 gles
      ...
      varyings vert ( uint id : SV_VertexID ){
        ...
        float size =  .005  *  clamp( length( v.vel) * 10000 , 0 , 1);
      ...
      float4 frag (varyings v) : COLOR {
        ...
        float val = .5 - length( v.uv - float2( .5 , .5 ));
        val *= 2;
        //val *= val;
      	
        if( val < .1 ){ discard;}
        float3 col = float3(val,val,val);//float3( 0,1,0);//v.nor * .5+ .5;// v.debug;

        return float4( v.debug * col * 4 , val );
