Shader "Custom/abberationWithCubemap" {
    ...
    Pass {
      ...
      fixed4 frag(VertexOut v) : COLOR {
        ...
        float3 dir = normalize((_WorldSpaceCameraPos - v.mPos));
      	...
      	float3 reflCol = float3(colR.x,colG.y,colB.z);

      	//float3 refl = normalize(reflect( -dir, v.normal ));
      	//v.normal.y *= -1;
      	float3 refl = normalize(refract( -dir, v.normal , .95));
      	float3 cubeCol = texCUBE( _Cube ,  refl );

        //Getting match of normal and cam dir for 'rim lighting'
        float match = dot( dir , v.normal );

        float3 aCol = tex2D( _Audio , float2(match,0)).xyz;
        //col += (1-match) * (1-match);
       ...
       // col = lerp( col , colMain , val  );
      	...
      	float3 col = cubeCol * aCol;

      	col = lerp( reflCol , col , min( length( col )* 3, 1) );
